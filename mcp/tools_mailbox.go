...
// registerMailboxTools registers mailbox management tools.
func registerMailboxTools(s *Server, cfg ToolsConfig) {
	s.RegisterTool(
		NewTool("mailbox_list", "List all mailbox folders with unread/total counts"),
		makeMailboxListHandler(cfg),
	)
	s.RegisterTool(
		NewTool("mailbox_create", "Create a new mailbox folder").
			WithProperty("name", "string", "Name for the new mailbox").
			WithProperty("parent_id", "string", "Parent mailbox ID for nested folders (optional)").
			WithRequired("name"),
		makeMailboxCreateHandler(cfg),
	)
	s.RegisterTool(
		NewTool("mailbox_rename", "Rename an existing mailbox folder").
			WithProperty("id", "string", "The mailbox ID").
			WithProperty("name", "string", "New name for the mailbox").
			WithRequired("id", "name"),
		makeMailboxRenameHandler(cfg),
	)
	s.RegisterTool(
		NewTool("mailbox_delete", "Delete a mailbox folder by ID").
			WithProperty("id", "string", "The mailbox ID").
			WithRequired("id"),
	...
	return func(ctx context.Context, _ map[string]any) (any, error) {
		...
		for i, mb := range mailboxes {
			result[i] = map[string]any{
				...
				"total_threads":  mb.TotalThreads,
				"unread_threads": mb.UnreadThreads,
			}
		}
		return result, nil
	}
...
func makeMailboxCreateHandler(cfg ToolsConfig) ToolHandler {
	return func(ctx context.Context, args map[string]any) (any, error) {
		...
		mailbox, err := cfg.Client.Mailbox().Create(ctx, name, parentID)
		if err != nil {
