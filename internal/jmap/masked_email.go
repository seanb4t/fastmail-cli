...
// MaskedEmailCapability is the JMAP capability URI for Fastmail's masked email extension.
...
// The clientID is a temporary identifier used in the response to match created objects.
func (b *MaskedEmailSetBuilder) Create(clientID string, email *MaskedEmail) *MaskedEmailSetBuilder {
	obj := make(map[string]any)
	if email.ForDomain != "" {
		obj["forDomain"] = email.ForDomain
	}
	if email.Description != "" {
		obj["description"] = email.Description
	}
	if email.URL != "" {
		obj["url"] = email.URL
	}
	if email.State != "" {
		obj["state"] = email.State
	}
	b.create[clientID] = obj
	return b
}

// Update adds a masked email to be updated.
func (b *MaskedEmailSetBuilder) Update(id string, changes map[string]any) *MaskedEmailSetBuilder {
	b.update[id] = changes
	return b
}
...
// SetError represents an error that occurred during a set operation.
